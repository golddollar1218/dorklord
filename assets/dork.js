...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function applyScrollDrivenTransformation(element, rect, windowHeight) {
        ...
        const transformationValue = parseFloat(element.getAttribute('data-transform-value'));
        ...
        if (transformationType && !isNaN(transformationValue)) {
            ...
            const progress = Math.min(Math.max((windowHeight - rect.top) / (windowHeight * 0.6 + rect.height), 0), 1);
    ...
    function applyTypingEffect(element) {
        ...
        function typeNextChar() {
            if (index < fullText.length) {
                ...
                let typingSpeed = Math.floor(Math.random() * (typingSpeedMax - typingSpeedMin + 1)) + typingSpeedMin;
