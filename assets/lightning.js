...
Lightning.prototype = {
    ...
    draw: function(ctx) {
        ...
        var gradient = ctx.createLinearGradient(points[0].x, points[0].y, points[points.length - 1].x, points[points.length - 1].y);
    ...
    _updateStepsByParent: function() {
        ...
        this.endStep   = this.startStep + randint(Math.max(parentStep - this.startStep - 2, 1)) + 2;
...
function LightningEffect(canvasId, config) {
    ...
    function updateLightningPositions() {
        ...
        for (var i = 0; i < config.lightning.count; i++) {
            var startRel = config.lightning.startPoints[i % config.lightning.startPoints.length];
            var endRel = config.lightning.endPoints[i % config.lightning.endPoints.length];
            ...
            lightning.lineWidth = lightning.getResponsiveValue(config.lightning.thickness);
            lightning.blur = lightning.getResponsiveValue(config.lightning.blur);
            lightning.color = 'rgb(' + config.lightning.color.join(',') + ')';
            lightning.blurColor = 'rgba(' + config.lightning.color.join(',') + ', 0.5)';
